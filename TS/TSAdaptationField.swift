...
class TSAdaptationField {
    ...
    func dump() {
        ...
        print(String(format: """
                adaptationFieldLength  : %02X \
                discontinuityIndicator : %02X \
                randomAccessIndicator : %02X \
                elementaryStreamPriorityIndicator : %02X \
                PCR_flag : %02X \
                OPCR_flag : %02X \
                splicingPoint_flag : %02X \
                transportPrivateData_flag : %02X \
                adaptationFieldExtension_flag : %02X
            """, length,
    ...
    var data: Data {
        ...
        set {
            ...
            do {
                ...
                if PCRFlag {
                    PCR = try buffer.readBytes(Int(TSAdaptationField.PCRSize))
                }
                if OPCRFlag {
                    OPCR = try buffer.readBytes(Int(TSAdaptationField.PCRSize))
                }
                if splicingPointFlag {

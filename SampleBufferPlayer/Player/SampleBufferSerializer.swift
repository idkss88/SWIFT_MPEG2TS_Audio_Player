...
class SampleBufferSerializer  {
    ...
    var nextSampleOffset = CMTime.zero
    ...
    private func provideMoreMediaData() {
        ...
        while audioRenderer.isReadyForMoreMediaData {
            ...
            if let sampleBuffer = currentItem.nextSampleBuffer() {
                ...
                print("self.nextSampleOffset scaled \(audioClock?.convertTime(pts, to: audioRenderer.timebase))")
                ...
                let pts2 = CMSampleBufferGetOutputPresentationTimeStamp(sampleBuffer)
                print("test2 get pts is \(pts2) ")
                let duration = CMSampleBufferGetOutputDuration(sampleBuffer)
                print("test2 get duration is \(duration.seconds) ")
                // Feed the sample buffer to the renderer.
                nextSampleOffset = pts2 + duration
                print("render time \(renderSynchronizer.timebase.time.seconds) sampleBuffer time \(sampleBuffer.presentationTimeStamp.seconds)  ")
    ...
    private func notifyPlaybackRateChanged(from oldRate: Float) {
        print("notifyPlaybackRateChanged")
    ...
    private func flushItem(_ item: SampleBufferItem) {
        ...
        renderSynchronizer.setRate(1.0, time: CMTimeMake(value: -1, timescale: 1))
        item.flushSource()
    }
}

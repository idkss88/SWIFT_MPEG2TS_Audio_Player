...
class SampleBufferItem {
    ...
    private var aacSource: AC3SampleBufferSource?
    ...
    init(playlistItem: PlaylistItem,
        ...
        if playlistItem.ext == "aac" || playlistItem.ext == "ac3" {
            self.testItem = true
        }
    }
    ...
    func nextSampleBuffer() -> CMSampleBuffer? {
        print("idkss88 - nextSampleBuffer 0")
        ...
        do {
            ...
            if aacSource == nil && sampleBufferSource == nil {
                isEnqueued = true
                if self.testItem {
                    print("startOffset is \(startOffset)")
                    //aacSource = AC3SampleBufferSource(fileURL: playlistItem.url, fromOffset: startOffset)
                    aacSource = AC3SampleBufferSource(fromOffset: startOffset)
                  // aacSource = ADTSSampleBufferSource(fileURL: playlistItem.url, fromOffset: startOffset)
                    //aacSource = ADTSSampleBufferSource(fromOffset: startOffset)
                    
                   // aacSource = ADTSSampleBufferSource2()
                   // print("aacSource delate \(aacSource?.demuxer.delegate)")
                    
                  //  aacSource?.demuxer.downloader.ip = "233.15.200.55"
                   // aacSource?.demuxer.delegate = aacSource
                } else {
                    print("startOffset2 is \(startOffset)")
            ...
            if let source = sampleBufferSource {
                print("idkss88 - nextSampleBuffer 1")
                ...
                //idkss88
                print("idkss88 - nextSampleBuffer 2")
                let sampleBuffer = try source.nextSampleBuffer()
    ...
    func flushSource() {
        ...
        //aacSource = nil
        aacSource?.reader?.currentPacket = 0
    ...
    func invalidateSource() {
        print("invalidateSource")
        sampleBufferSource = nil
        //aacSource?.reader = nil
        aacSource = nil

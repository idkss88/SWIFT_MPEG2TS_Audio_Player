...
func ReaderConverterCallback(_ converter: AudioConverterRef,
    ...
    print("idkss88 packetIndex \(packetIndex)")
    ...
    if isEndOfData {
        if reader.parser.isParsingComplete {
            ...
            print("idkss88 ReaderReachedEndOfDataError \(packetIndex)")
            return ReaderReachedEndOfDataError
        } else {
            print("idkss88 ReaderNotEnoughDataError \(packetIndex)")
    ...
    print("mDataByteSize \(UInt32(dataCount))")
    //
    ...
    if sourceFormatDescription.mFormatID != kAudioFormatLinearPCM {
        ...
        print("out mDataByteSize \(UInt32(dataCount))")
    ...
    print("idkss88 outPacketDescriptions?.pointee?.pointee.mDataByteSize \(outPacketDescriptions?.pointee?.pointee.mDataByteSize)")

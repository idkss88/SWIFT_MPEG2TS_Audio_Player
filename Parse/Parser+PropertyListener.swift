...
func ParserPropertyChangeCallback(_ context: UnsafeMutableRawPointer, _ streamID: AudioFileStreamID, _ propertyID: AudioFileStreamPropertyID, _ flags: UnsafeMutablePointer<AudioFileStreamPropertyFlags>) {
    ...
    case kAudioFileStreamProperty_DataOffset:
        var size: UInt32 = 0
        var error = AudioFileStreamGetPropertyInfo(streamID, propertyID, &size, nil)
        guard error == noErr && size != 0 else {
            print("Error kAudioFileStreamProperty_DataOffset")
            return
        }
        
        var dataOffset: [CChar] = Array(repeating: 0, count: Int(size))
        error = AudioFileStreamGetProperty(streamID, kAudioFilePropertyChannelLayout, &size,  &dataOffset)
    
        print("offset : \(dataOffset)")
    case kAudioFileStreamProperty_ChannelLayout:
        var layoutSize: UInt32 = 0
        var error = AudioFileStreamGetPropertyInfo(streamID, propertyID, &layoutSize, nil)
        guard error == noErr && layoutSize != 0 else {
            print("Error AudioFileStreamGetPropertyInfo")
            return
        }
        
        var layout: [CChar] = Array(repeating: 0, count: Int(layoutSize))
        error = AudioFileStreamGetProperty(streamID, kAudioFilePropertyChannelLayout, &layoutSize,  &layout)
    
        parser.audioChannelLayout = layout
        parser.audioChannelLayoutSize = layoutSize
        
    case kAudioFilePropertyMagicCookieData:
        print("kAudioFilePropertyMagicCookieData")
        
        // Grab the cookie from the source file and set it on the converter.
        var cookieSize: UInt32 = 0
        var error = AudioFileStreamGetPropertyInfo(streamID, propertyID, &cookieSize, nil)
        ...
        error = AudioFileStreamGetProperty(streamID, propertyID, &cookieSize, &cookie)
